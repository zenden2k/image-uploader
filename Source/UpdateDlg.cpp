...
bool CanWriteToFolder(CString folder)
{
   HANDLE hFile = ::CreateFile(folder, GENERIC_WRITE, FILE_SHARE_READ, NULL, 
    ...
    /*hFile = ::CreateFile("\\\\192.168.0.1\\новая папка", GENERIC_READ, FILE_SHARE_READ, NULL, 
                         OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);

    if (hFile == INVALID_HANDLE_VALUE) 
    { 
        printf("Could not open file for read (error %d)\n", GetLastError());
    }
    else
    {
...
LRESULT CUpdateDlg::OnClickedOK(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	if(!m_bUpdateFinished)
	{
		...
		// Begin Update Process
			...
			NeedElevation |= isVista && !elev && !CmdLine.IsOption(_T("update")) && !CanWriteToFolder(IU_GetDataFolder());
			if(NeedElevation)
			{
	...
	else
	{
		...
		//m_bClose = true;
		m_bClose = 2;
		//::SendMessage(GetParent(),WM_MY_EXIT,5,0);
		return 0;	
	}
...
void CUpdateDlg::DoUpdates()
{
	...
	if(m_UpdateManager.m_nSuccessPackageUpdates)
	{
...
void CUpdateDlg::Abort()
{
	if(IsRunning())
	{
		...
		//Sleep(1000);
		Terminate();
		//MsgWaitForSingleObject(m_hThread, 2500);
	}
}
