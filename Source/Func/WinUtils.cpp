...
// —оздание €рлыка
// ¬ходные параметры:
//  pwzShortCutFileName - путь и им€ €рлыка, например, "C:\\Ѕлокнот.lnk"
//  ≈сли не указан путь, €рлык будет создан в папке, указанной в следующем параметре.
//  ѕрим.: Windows сама Ќ≈ добавл€ет к имени расширение .lnk
//  pszPathAndFileName  - путь и им€ exe-файла, например, "C:\\Windows\\NotePad.Exe"
//  pszWorkingDirectory - рабочий каталог, например, "C:\\Windows"
//  pszArguments        - аргументы командной строки, например, "C:\\Doc\\Text.Txt"
//  wHotKey             - гор€ча€ клавиша, например, дл€ Ctrl+Alt+A     HOTKEY(HOTKEYF_ALT|HOTKEYF_CONTROL,'A')
//  iCmdShow            - начальный вид, например, SW_SHOWNORMAL
//  pszIconFileName     - путь и им€ файла, содержащего иконку, например, "C:\\Windows\\NotePad.Exe"
//  int iIconIndex      - индекс иконки в файле, нумеруетс€ с 0
bool CreateShortCut(
                                         ...
                                         int iIconIndex)
{
    ...
    // ѕолучение экземпл€ра компонента "ярлык"
...
float GetMonitorScaleFactor()
{
    ...
    int LogicalMonitorWidth = LogicalMonitorInfo.rcMonitor.right Ц LogicalMonitorInfo.rcMonitor.left;
