...
public:
    ...
    bool open(const std::string& fileName) {
        ...
        if (avformat_find_stream_info(pFormatCtx, nullptr) < 0) {
            throw FrameGrabberException("Couldn't find stream information.");
        }
        ...
        if ((ret = avcodec_parameters_to_context(pCodecCtx, pars)) < 0) {
            throw FrameGrabberException("Failed to copy video codec parameters to decoder context");
        }

        //pCodecCtx->refcounted_frames = 1;
        // Open codec
        if (avcodec_open2(pCodecCtx, pCodec, nullptr) < 0) {
            throw FrameGrabberException("Failed to open video codec");
        }
