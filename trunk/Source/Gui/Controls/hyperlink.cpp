 ...
 * Title    : CSpecialHyperLink definition.
 *
 ...
 *            - Added changes to make CSpecialHyperLink compatible with UNICODE
 ...
 */
COLORREF CSpecialHyperLink::g_crLinkColor     = RGB(  0,   0, 255);   // Blue;
COLORREF CSpecialHyperLink::g_crVisitedColor  = RGB( 128,  0, 128);   // Purple;
HCURSOR  CSpecialHyperLink::g_hLinkCursor     = NULL;
HFONT    CSpecialHyperLink::g_UnderlineFont   = NULL;
int      CSpecialHyperLink::g_counter         = 0;
...
CSpecialHyperLink::CSpecialHyperLink(void)
{
...
/*
 * Function CSpecialHyperLink::ConvertStaticToHyperlink
 */
BOOL CSpecialHyperLink::ConvertStaticToHyperlink(HWND hwndCtl, LPCTSTR strURL)
{
    if( !(setURL(strURL)) )
		return FALSE;
 ...
 * Function CSpecialHyperLink::ConvertStaticToHyperlink
 */
BOOL CSpecialHyperLink::ConvertStaticToHyperlink(HWND hwndParent, UINT uiCtlId,
										  LPCTSTR strURL)
{
...
/*
 * Function CSpecialHyperLink::setURL
 */
BOOL CSpecialHyperLink::setURL(LPCTSTR strURL)
{
	if( m_strURL )
	{
		delete [] m_strURL;
...
/*
 * Function CSpecialHyperLink::_HyperlinkParentProc
 */
LRESULT CALLBACK CSpecialHyperLink::_HyperlinkParentProc(HWND hwnd, UINT message,
	...
	case WM_CTLCOLORSTATIC:
		{
			...
			CSpecialHyperLink *pHyperLink = (CSpecialHyperLink *)GetProp(hwndCtl,
				                                           PROP_OBJECT_PTR);
			...
			if(pHyperLink)
			{
				...
				if (!pHyperLink->m_bVisited)
				{
					...
					// optimization
					SetTextColor(hdc, CSpecialHyperLink::g_crLinkColor);
				}
				else
				{
					SetTextColor(hdc, CSpecialHyperLink::g_crVisitedColor);
				}
				return lr;
			}
			break;
...
/*
 * Function CSpecialHyperLink::Navigate
 */
inline void CSpecialHyperLink::Navigate(void)
{
...
/*
 * Function CSpecialHyperLink::DrawFocusRect
 */
inline void CSpecialHyperLink::DrawFocusRect(HWND hwnd)
{
...
/*
 * Function CSpecialHyperLink::_HyperlinkProc
 *
 ...
 *       as expected.
 */
LRESULT CALLBACK CSpecialHyperLink::_HyperlinkProc(HWND hwnd, UINT message,
		                                   WPARAM wParam, LPARAM lParam)
{
	CSpecialHyperLink *pHyperLink = (CSpecialHyperLink *)GetProp(hwnd, PROP_OBJECT_PTR);
	...
	case WM_MOUSEMOVE:
		{
			...
			else
			{
				...
				SendMessage(hwnd, WM_SETFONT,
					        (WPARAM)CSpecialHyperLink::g_UnderlineFont, FALSE);
	...
	case WM_SETCURSOR:
		{
			SetCursor(CSpecialHyperLink::g_hLinkCursor);
			return TRUE;
		}
	...
	case WM_LBUTTONUP:
		{
			//SendMessage(GetParent(hwnd), WM_COMMAND, (WPARAM)MAKELONG(pHyperLink->m_commandID,BN_CLICKED),/*(LPARAM)hwnd*/0);
			//pHyperLink->Navigate();
			return 0;
		}
	...
	case WM_KILLFOCUS:
		{
			...
			CSpecialHyperLink::DrawFocusRect(hwnd);
			return 0;
		}
	case WM_DESTROY:
		{
			...
			if( --CSpecialHyperLink::g_counter <= 0 )
			{
	...
	return 
		pHyperLink->m_pfnOrigCtlProc(hwnd, message,
		                  wParam, lParam);
		/*CallWindowProc(pHyperLink->m_pfnOrigCtlProc, hwnd, message,
		                  wParam, lParam);*/
}

/*
 * Function CSpecialHyperLink::createUnderlineFont
 */
void CSpecialHyperLink::createUnderlineFont(void)
{
	LOGFONT lf;
...
/*
 * Function CSpecialHyperLink::createLinkCursor
 */
void CSpecialHyperLink::createLinkCursor(void)
{
...
void CSpecialHyperLink::setCommandID(int id)
{
	m_commandID = id;
}